...
BTTargetUnreachableAction.run = function (self, unit, blackboard, t, dt)
	...
	if whereabouts_extension then
		position_list, target_on_mesh = whereabouts_extension.closest_positions_when_outside_navmesh(whereabouts_extension)
	end
	...
	if closest_position then
		navigation_extension.move_to(navigation_extension, closest_position)
	end

	local locomotion_extension = blackboard.locomotion_extension

	self.move_closer(self, unit, blackboard, locomotion_extension, navigation_extension)
...
BTTargetUnreachableAction._debug_distance_text = function (self, unit, navigation_extension)
	if script_data.debug_ai_movement then
		...
		local distance = navigation_extension.distance_to_destination(navigation_extension, unit_position)
