...
BTClanRatFollowAction.enter = function (self, unit, blackboard, t)
	...
	local should_walk = self._should_walk(self, destination, position, enter_walk_dist_sq, rotation)
	local use_slow_approach = self._slow_approach(self, destination, position, action_data, rotation)
	...
	if should_walk or use_slow_approach then
		...
		local walk_anim, walk_dir = self._calculate_walk_animation(self, Quaternion.right(rotation), Quaternion.forward(rotation), dir, position)
...
BTClanRatFollowAction._update_walking = function (self, unit, blackboard, dt, t)
	...
	local walk_dir = self._calculate_walk_dir(self, Quaternion.right(rotation), Quaternion.forward(rotation), dir, self_pos, walk_anims)

	if walk_dir ~= blackboard.walking_direction then
		local walk_anim = self._calculate_walk_animation(self, walk_dir, walk_anims)
