...
QuestManager.init = function (self, statistics_db)
	...
	local quests = backend_interface_quests.get_quests(backend_interface_quests)
...
QuestManager.event_stat_incremented = function (self, stats_id, ...)
	...
	local stats = statistics_db.get_all_stats(statistics_db, stats_id)
	...
	for quest_key, quest_id in pairs(quests) do
		...
		for i = 1, #stat_mappings, 1 do
			...
			if success then
				...
				statistics_db.increment_stat(statistics_db, stats_id, "quest_statistics", stat_name)
...
QuestManager.update = function (self, dt, t)
	...
	if reward_poll_id then
		...
		local poll_completed = backend_interface_quests.quest_rewards_generated(backend_interface_quests, reward_poll_id)

		if poll_completed then
			local rewards = backend_interface_quests.get_quest_rewards(backend_interface_quests, reward_poll_id)
			...
			for i = 1, #stat_names, 1 do
				...
				statistics_db.set_stat(statistics_db, stats_id, "quest_statistics", stat_name, 0)
			end
	...
	if refresh_poll_id then
		local statistics_db = self._statistics_db
		local backend_interface_quests = self._backend_interface_quests
		local poll_completed, quest_key = backend_interface_quests.is_quest_refreshed(backend_interface_quests, refresh_poll_id)
...
QuestManager.refresh_quest = function (self, quest_id)
	...
	local quest_key = table.find(quests, quest_id)
	local backend_interface_quests = self._backend_interface_quests
	local refresh_poll_id = backend_interface_quests.refresh_quest(backend_interface_quests, quest_key)
...
QuestManager.claim_reward = function (self, quest_id)
	...
	local quest_key = table.find(quests, quest_id)
	local backend_interface_quests = self._backend_interface_quests
	local reward_poll_id = backend_interface_quests.claim_quest_rewards(backend_interface_quests, quest_key)
...
QuestManager.can_claim_quest_rewards = function (self, quest_id)
	...
	local quest_key = table.find(quests, quest_id)
	local backend_interface_quests = self._backend_interface_quests
	local can_claim = backend_interface_quests.can_claim_quest_rewards(backend_interface_quests, quest_key)
...
QuestManager.time_until_new_daily_quest = function (self)
	...
	local backend_interface_quests = self._backend_interface_quests
	local next_quest_time = backend_interface_quests.get_daily_quest_update_time(backend_interface_quests)
