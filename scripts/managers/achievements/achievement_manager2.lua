...
AchievementManager.update = function (self, dt, t)
	...
	if should_process then
		...
		if result then
			...
			if token then
				self._unlock_tasks[template.id] = token
	...
	self._curr_template_idx = self._curr_template_idx + 1

	if self._template_count < self._curr_template_idx then
		self._curr_template_idx = 1
	end

	for id, token in pairs(self._unlock_tasks) do
		local done, error_msg = self._platform_functions.unlock_result(token)

		if done then
			self._unlock_tasks[id] = nil

			if error_msg == nil then
				self._unlocked_achievements[id] = true
			else
