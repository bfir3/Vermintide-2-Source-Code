...
WeaponSkins.debug_verify_skins = function ()
	...
	for name, skin in pairs(WeaponSkins.skins) do
		...
		if skin.left_hand_unit then
			local id = rawget(NetworkLookup.inventory_packages, skin.left_hand_unit)

			if not id then
				local s = sprintf("Weapon Skin '%s' has a left hand unit (%s) that does not exist in inventory_package_list.lua", name, skin.left_hand_unit)
		...
		if skin.right_hand_unit then
			local id = rawget(NetworkLookup.inventory_packages, skin.right_hand_unit)

			if not id then
				local s = sprintf("Weapon Skin '%s' has a right hand unit (%s) that does not exist in inventory_package_list.lua", name, skin.right_hand_unit)
